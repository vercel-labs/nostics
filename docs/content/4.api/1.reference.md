...
Builds a console reporter. `createReporterLog()` uses `console.warn` and `formatDiagnostic`; `method` is overridable per call. `reporterLog` is the deprecated alias for `createReporterLog()`.
