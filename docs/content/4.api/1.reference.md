...
Lean production counterpart to `defineDiagnostics`. Returns a `Proxy` that builds a minimal `Diagnostic` for any accessed code: the code becomes the `message` (`why`), `name` stays the default `Diagnostic`, and `docs` is derived from `docsBase`. It carries no catalog text, so it stays tiny in a bundle.
