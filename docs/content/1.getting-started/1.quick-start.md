...
Install the nostics plugin to give Claude agent skills for your diagnostic catalog:

```bash
claude plugin add https://github.com/vercel-labs/nostics
```

Claude will automatically pick up the `nostics` API reference and an `add-diagnostic` skill that finds the right catalog, chooses the next free code, and wires the call site. Claude will automatically pick up the `nostics` API reference and an `add-diagnostic` skill that finds the right catalog, chooses the next free code, and wires the call site.
