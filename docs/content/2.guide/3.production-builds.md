...
[`defineProdDiagnostics`](/api/reference#defineproddiagnosticsoptions) returns a `Proxy` that answers for any code you access. The code becomes the instance `name`, `docs` is still derived from `docsBase`, and the `message` (`why`) points to the docs URL (empty when there is no `docsBase`, so the thrown header is just the code):

```ts
throw diagnostics.NUXT_B2011() // NUXT_B2011: https://docs.example.com/nuxt_b2011
```

So a thrown diagnostic keeps its code (as the `name`) and a working docs link, but no catalog text reaches the bundle. It does not keep definition data, so `diagnostic.data` is `undefined` in this branch. In a development and production ternary, TypeScript exposes the development data type as optional.
