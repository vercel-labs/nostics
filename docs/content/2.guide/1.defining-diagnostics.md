...
description: Declare stable codes, typed params, structured data, docs links, sources, and causes.
navigation:
...
export const diagnostics = defineDiagnostics({
  ...
  codes: {
    NUXT_B2011: {
      ...
      data: (p: { src: string, mode: 'client' | 'server' }) => ({
        src: p.src,
        mode: p.mode,
      }),
    },
...
`why`, `fix`, and `data` can use functions. Function params are inferred and required at the call site.
...
Params from `why`, `fix`, and `data` are combined. In the example above, `value`, `configPath`, and `example` are required.

## Structured data

Use `data` for machine-readable information that depends on the diagnostic code. It can be a static object or a function of the shared params.

```ts
const diagnostic = defineDiagnostics({
  codes: {
    NUXT_B5001: {
      why: (p: { value: string }) => `Invalid value "${p.value}".`,
      data: (p: { value: string }) => ({
        received: p.value,
        expected: 'ISO date' as const,
      }),
    },
  },
}).NUXT_B5001({ value: 'today' })

diagnostic.data.received // string
diagnostic.data.expected // "ISO date"
```

Each code keeps its own `data` type. `toJSON()` includes the resolved object, so JSON reporters keep it too. Use JSON-compatible values when a reporter serializes the diagnostic.
...
Pick a format and keep it stable. We recommend a structured shape that scales across many codes:
...
This shape is only a convention, not a requirement. A code is any string, so an explicit name like `invalidDate` works just as well and reads clearer when you only have a few:
...
diagnostics.invalidDate({ value, example })
diagnostics.missingConfig({ path })
```
