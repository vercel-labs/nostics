...
::warning
A diagnostic only reaches the console (or anywhere else) through a reporter. With no `reporters` configured, `diagnostics.NUXT_B2011()` just builds and returns a `Diagnostic`, it prints nothing. Add a reporter such as `createConsoleReporter()` to report it, or `throw` the returned diagnostic to surface it as an error.
::
...
The reporter renders each diagnostic with a `formatter`, defaulting to [`formatDiagnostic`](#formatters). Pass your own to change how the diagnostic is printed, for example to add color in a terminal with `ansiFormatter`:
...
Any function with the `(diagnostic: Diagnostic) => string` shape works, so you can also reuse `jsonFormatter` or write your own.
...
`formatDiagnostic` emits this layout as plain text. Swap in `ansiFormatter` and the same diagnostic gains color when printed to a terminal, rendered above as it would appear there.
