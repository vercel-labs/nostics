...
| Reporter | Import | Use |
| --- | --- | --- |
| `createReporterLog(options?)` | `nostics` | Console reporter; defaults to `console.warn` + `formatDiagnostic` |
...
`createReporterLog()` uses `console.warn` by default. Pass a method when you call the diagnostic if you need a different console method.
...
```ts
const reporterAlwaysError = createReporterLog({ method: 'error' })
```
